...
f_line_0 = np.array([[-0.35,1],[2,-0.5]])


x = np.linspace(-1,1.5,1000)[::-1]
y = np.sin(3*x)*0.48
...
voronoi = scipy.spatial.Voronoi(node_coords.reshape((-1,3))[:,:2])  # type: ignore
ax = plt.axes()
